...
/**
 * FarmerChatRoom Component - Handles chat functionality for farmers
 * Features WhatsApp-style message alignment and real-time messaging
 */
function FarmerChatRoom() {
  ...
  const setupWebSocketConnection = (token) => {
    ...
    socket.onmessage = (event) => {
      ...
      if (data.type === 'message') {
        ...
        if (data.user_id !== currentUserId && roomDetails?.customer?.user_id === data.user_id) {
          setCustomerOnline(data.is_online);
        }
      } else if (data.is_online !== undefined) {
        // Direct online/offline status update
        if (roomDetails?.customer?.user_id !== currentUserId) {
          setCustomerOnline(data.is_online);
        }
      }
    };
        ...
        {loading ? (
          ...
          <div className="space-y-4">
            {messages.map((msg, index) => {
              ...
              const displayName = isCurrentUser ? 'You' : (msg.sender_name || 'Customer');
